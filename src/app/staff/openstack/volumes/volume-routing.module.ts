...
import { VolumeBackupsResolver } from '@fleio-api/openstack/volume/volume-backups.resolver';
import { VolumeSnapshotsResolver } from '@fleio-api/openstack/volume/volume-snapshots.resolver';
...
const routes: Routes = [
  ...
  },
  {
    ...
    resolve: {
      ...
      backups: VolumeBackupsResolver,
      snapshots: VolumeSnapshotsResolver,
    },
    data: {
      config: {
        getBreadCrumbDetail: (data) => {
          return `Volume ${data.volume.name || data.volume.id}`;
        },
      } as IRouteConfig,
    },
    runGuardsAndResolvers: 'always',
