...
import { UserDetailsGroupsComponent } from '@shared/common-tabs/clients-users/users/user-details-groups/user-details-groups.component';
import { UserDetailsTicketsComponent } from '@shared/common-tabs/clients-users/users/user-details-tickets/user-details-tickets.component';
...
export class UserUIService extends ObjectUIServiceBase<IUserModel> {
  ...
  getActions(): IAction[] {
    ...
    if (!this.object.is_superuser && !this.object.is_staff) {
      actions.push(new RouterLinkAction({
          icon: {name: 'face'},
    ...
    actions.push(new ApiCallAction(
      {
        ...
        errorMessage: 'Failed to delete user, check logs for details',
  ...
  getCardFields(): IDataField[] {
    ...
    let clientDetails;
    if (this.object.clients.length === 1) {
      clientDetails = {
        name: 'Client',
        value: this.object.clients[0].name
      }
    } else if (this.object.clients.length > 1) {
      clientDetails = {
        name: 'Clients count',
        value: `${this.object.clients.length} clients`
      }
    }
    const fields = [
      {
    ...
    if (clientDetails) {
      fields.push(clientDetails);
    }
    if (this.object.reseller_client_details) {
      fields.push({
        name: 'Reseller',
        value: this.object.reseller_client_details.name
      })
    }
    return fields;
  }
    ...
    switch (this.state) {
      case 'details':
        return [
          ...
          {
            tabName: 'User groups',
            component: UserDetailsGroupsComponent
          },
          {
            tabName: 'Tickets',
            featureName: 'plugins.tickets',
            component: UserDetailsTicketsComponent
          }
        ];
      case 'edit':
      case 'create':
  ...
  getCardTags(): string[] {
    ...
    if (this.object.is_superuser) {
      tags.push('superuser');
    }
    if (!this.object.is_active) {
      tags.push('inactive');
    }
    return tags;
  }
