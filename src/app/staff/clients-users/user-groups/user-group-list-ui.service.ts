...
import { UserGroupUiService } from './user-group-ui.service';
...
export class UserGroupListUiService implements IObjectListUIService {
  public noItemsMessage = 'No user groups';
  ...
  getObjectUIService(
    ...
    return new UserGroupUiService(
      object as IUserGroupModel, permissions, state, this.router, this.config, this.userGroupsApiService,
    );
  }

  getTableData(objectList: FleioObjectsList<IUserGroupModel>): ITableData {
    ...
    for (const userGroup of objectList.objects) {
      const rowUIService = this.getObjectUIService(userGroup, objectList.permissions, 'table-view');
      const row: ITableRow = {
        cells: {
          name: {text: userGroup.name},
          created_at: {text: this.datePipe.transform(userGroup.created_at)},
  ...
  getActions(objectList: FleioObjectsList<IBaseFleioObjectModel>): IAction[] {
    return [
      new RouterLinkAction({
        ...
        tooltip: 'Create user group',
        icon: {name: 'add'},
        router: this.router,
        routerUrl: this.config.getPanelUrl('clients-users/user-groups/create')
      })
    ];
  }
