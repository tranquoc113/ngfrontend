    ...
    const tableData: ITableData = {
      header: {
        columns: [
          ...
          {type: ColumnType.Value, displayName: 'Used by reseller', enableSort: false, fieldName: 'reseller_resources'},
          {type: ColumnType.Actions, displayName: 'Actions', enableSort: false, fieldName: '(actions)'},
        ],
        columnNames: ['name', 'tags', 'description', 'reseller_resources', '(actions)'],
        showStatus: true,
      },
      rows: [],
    };
    ...
    for (const configuration of objectList.objects) {
      ...
      const row: ITableRow = {
        cells: {
          ...
          reseller_resources: {text: configuration.reseller_resources ? 'yes' : ''},
